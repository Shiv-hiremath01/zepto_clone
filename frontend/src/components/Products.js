...
const Products = ({ user, setUser, refreshUser }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://3.110.43.201:5000/api/products', {
          params: {
            category: selectedCategory === 'All' ? '' : selectedCategory,
            subcategory: selectedSubcategory,
            query: searchQuery,
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      await axios.post('http://3.110.43.201:5000/api/cart/add', {
        productId,
        quantity: 1,
      }, {
        headers: { Authorization: `Bearer ${user.token}` },
  ...
  // Rest of the code (JSX and styles) remains unchanged
  return (
    <div className="products-container">
      <NavMenu setSelectedCategory={setSelectedCategory} page="home" />
      <div className="product-menu-container">
